...
def is_executable_python(code: str) -> bool:
    ...
    if code.strip().startswith('class '):
        ...
        for line in code.split('\n'):
            ...
            if (stripped and not stripped.startswith(('class ', 'def ', '@', '#', ')'))
        ...
        for block in bash_blocks:
            ...
            for line in lines:
                ...
                try:
                    ...
                    if (result.returncode != 0 and 'Troubleshooting' not in content
                        and '--help' not in cmd and '--version' not in cmd):
                        pytest.fail(
        ...
        for doc_file in get_doc_files():
            ...
            for old_name in deprecated:
                assert old_name not in content, (
                    f"Deprecated '{old_name}' found in {doc_file.name}"
                )
